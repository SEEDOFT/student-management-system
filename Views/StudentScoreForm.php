...
<div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden mb-8">
        <div class="bg-gradient-to-r from-blue-500 to-blue-600 p-6">
            <h2 class="text-2xl font-bold text-white">Enter Student Scores</h2>
            <p class="text-sm text-blue-100 mt-1">Select faculty, year, and semester to find subjects and students.</p>
        </div>
        <div class="p-6">
            <form action="StudentScoreForm.php" method="GET" class="space-y-4">
                ...
                <?php if (!empty($subjects)): ?>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-end pt-4 border-t mt-4">
                        ...
                        <div>
                            <button type="submit"
                                class="w-full px-6 py-2 bg-blue-600 text-white font-semibold rounded-md shadow-sm hover:bg-blue-700">Load
                                Students</button>
                        </div>
                    </div>
                <?php else: ?>
                    <div class="pt-4 border-t mt-4">
                        <button type="submit"
                            class="w-full md:w-auto px-6 py-2 bg-gray-600 text-white font-semibold rounded-md shadow-sm hover:bg-gray-700">Find
                            Subjects</button>
                    </div>
                <?php endif; ?>
            </form>
    ...
    <?php if (!empty($students) && $selected_sub_id > 0): ?>
        <div class="bg-white rounded-lg shadow-md mt-8">
            <form action="../Controllers/ScoreController.php" method="POST">
                ...
                <div class="p-6 border-b flex justify-between items-center">
                    <h3 class="text-lg font-semibold text-gray-800">Entering Scores for: <span
                            class="text-blue-600"><?php echo htmlspecialchars($subject_name); ?></span></h3>
                    <div>
                        <label for="regDate" class="sr-only">Record Date</label>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr>
                            ...
                            <th class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 w-1/4">Score (0-100)</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        <?php foreach ($students as $student): ?>
                            <tr>
                                ...
                                <td class="px-3 py-2"><input type="number" name="scores[<?php echo $student['stuId']; ?>]"
                                        min="0" max="100" step="0.1" class="w-full px-3 py-1 border border-gray-300 rounded-md">
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                ...
                <div class="p-6 bg-gray-50 border-t flex justify-end">
                    <button type="submit"
                        class="px-6 py-2 bg-blue-600 text-white font-semibold rounded-md shadow-sm hover:bg-blue-700">Submit
                        Scores</button>
                </div>
            </form>
        </div>
